...
public class staffms implements ActionListener {
    ...
    public class department_name{
        ...
        public void allotJob(request req) {
            try {
                ps = con.prepareStatement("select processed_by, count(*) from request where department_name = ? group by processed_by having processed_by is not null");
                ps.setInt(1,req.department_name);
                rs = ps.executeQuery();
                rs.next();
    ...
    department_name alldepartment_names[];
    ...
    private ResultSet rs = null;
    {
        try {
            ...
            con = DriverManager.getConnection("jdbc:postgresql://localhost:5433/kaag", "postgres", "postgres");
    ...
    JComboBox requestPagedepartment_nameComboBox ;
    ...
    JTextArea persons,staff,department_name,IStaff,requests,IPageRequests,HPageRequests;
    ...
    public void clearAll() {
        ...
        requestPagedepartment_nameComboBox.setBounds(0,0,0,0);
    ...
    public void loadAdminLoginPage() {
        ...
        ALoginPageMessage.setText("password : kirioli");
    }
    public void loadRequestPlacingPage() {
        ...
        requestPagedepartment_nameComboBox.setBounds(600,100+70*2+13,400,40);
    }
    ...
    public void loadAdminPage() {
        ...
        try {
            ...
            persons.setText(k);
            k="All department_names\n";
            k+=get10("departno");
            k+=get10("dept name");
            k+=get10("HOD");
            k+="\n\n";
            for(int i=0;i<alldepartment_names.length;i++) {
                k+=get10(Integer.toString(alldepartment_names[i].deptNo));
                k+=get10(alldepartment_names[i].deptName);
                k+=get10(Integer.toString(alldepartment_names[i].hod));
                k+="\n";
            }
            department_name.setText(k);
            ...
            for(int i=0;i<allRequests.size();i++) {
                ...
                k+="        ";
                k += get20(Integer.toString(allRequests.elementAt(i).department_name));
                k+="\n";
            }
            requests.setText(k);
    ...
    public void loadApprovalPage(int id) {
        ...
        currentHLogin = id;
        try {
            ps = con.prepareStatement("select request_no from request where department_name = ? and status = ?");
            ps.setInt(1,id);
            ps.setString(2,"WA");
            rs = ps.executeQuery();
    ...
    staffms(){
        try {
            ...
            while(rs.next()) {
                ...
                Person.department_name = rs.getInt("department_name");
            ...
            alldepartment_names = new department_name[getIntFromResultSet(st.executeQuery("select count(*) from department_name"))];
            rs = st.executeQuery("select * from department_name");
            count = 0;
            while(rs.next()) {
                department_name department_name = new department_name();
                department_name.deptNo = rs.getInt("dept_no");
                department_name.deptName = rs.getString("dept_name");
                department_name.hod = rs.getInt("HOD");
                alldepartment_names[count++] = department_name;
            }
            ...
            while(rs.next()) {
                ...
                Request.department_name = rs.getInt("department_name");
        ...
        ImageIcon background=new ImageIcon("C:\\Users\\aswin\\Downloads\\Telegram Desktop\\hc\\THOD\\ICTS-staff-management-system\\amn.jpg");
        ...
        for(int i=0;i<6;i++) {
            ...
            homeButtons[i].setBounds(0, 0, 30, 25);
            homeButtons[i].setBorder(new RoundedBorder(3));
            homeButtons[i].setFont(new Font("verdana",Font.BOLD,22));
            f.add(homeButtons[i]);
        ...
        for(int i=0;i<3;i++) {
            if(i!=2) {
                ...
                requestPageInputs[i].setFont(new Font("verdana",Font.PLAIN,18));
            }
            requestPageLabels[i] = new JLabel();
            requestPageLabels[i].setFont(new Font("verdana",Font.PLAIN,18));
        ...
        for(int i=0;i<11;i++) {
            ...
            requestDetailsPageLabel1[i].setFont(new Font("verdana",Font.PLAIN,16));
            requestDetailsPageLabel2[i].setFont(new Font("verdana",Font.PLAIN,16));
            f.add(requestDetailsPageLabel1[i]);
            f.add(requestDetailsPageLabel2[i]);
        }
        passwordMessage = new JLabel("               -- The earnest aspirant gains supreme wisdom --");
        passwordMessage.setFont(new Font("Georgia",Font.BOLD,18));
        f.add(passwordMessage);
        approveDenyButton = new JButton("Deny");
        approveDenyButton.setFont(new Font("verdana",Font.BOLD,18));
        approveDenyButton.addActionListener(this);
        f.add(approveDenyButton);
        approveRequestsButton = new JButton("Approve");
        approveRequestsButton.setFont(new Font("verdana",Font.BOLD,18));
        approveRequestsButton.addActionListener(this);
        f.add(approveRequestsButton);
        approveRequestsMessage = new JLabel();
        approveRequestsMessage.setFont(new Font("verdana",Font.ITALIC,16));
        f.add(approveRequestsMessage);
        approveRequestsInput = new JTextField();
        approveRequestsInput.setFont(new Font("verdana",Font.PLAIN,18));
        f.add(approveRequestsInput);
        approveRequestsLabel = new JLabel("Enter the processing completed requests request number(s) : (Eg:11,12)");
        approveRequestsLabel.setFont(new Font("verdana",Font.PLAIN,16));
        f.add(approveRequestsLabel);
        processRequestsButton = new JButton("OK");
        processRequestsButton.setFont(new Font("verdana",Font.BOLD,18));
        processRequestsButton.addActionListener(this);
        f.add(processRequestsButton);
        processRequestsMessage = new JLabel();
        processRequestsMessage.setFont(new Font("verdana",Font.ITALIC,16));
        f.add(processRequestsMessage);
        processRequestsInput = new JTextField();
        processRequestsInput.setFont(new Font("verdana",Font.PLAIN,18));
        f.add(processRequestsInput);
        processRequestsLabel = new JLabel("Enter the processing completed requests request number(s) : (Eg:11,12)");
        processRequestsLabel.setFont(new Font("verdana",Font.PLAIN,16));
        f.add(processRequestsLabel);
        IPageRequests = new JTextArea();
        HPageRequests = new JTextArea();
        IPageRequests.setFont(new Font("verdana",Font.PLAIN,18));
        HPageRequests.setFont(new Font("verdana",Font.PLAIN,18));
        ...
        department_name = new JTextArea();
        requests = new JTextArea();
        persons = new JTextArea();
        persons.setFont(new Font("verdana",Font.PLAIN,14));
        staff.setFont(new Font("verdana",Font.PLAIN,14));
        department_name.setFont(new Font("verdana",Font.PLAIN,14));
        IStaff.setFont(new Font("verdana",Font.PLAIN,14));
        requests.setFont(new Font("verdana",Font.PLAIN,14));
        ...
        AScroll[1] = new JScrollPane(department_name);
        ...
        ALabel.setFont(new Font("verdana",Font.BOLD,20));
        f.add(ALabel);
        ...
        ALoginPageLabel.setFont(new Font("verdana",Font.PLAIN,18));
        ALoginPageInput.setFont(new Font("verdana",Font.PLAIN,18));
        ALoginPageButton.setFont(new Font("verdana",Font.BOLD,18));
        ALoginPageMessage.setFont(new Font("verdana",Font.ITALIC,18));
        ...
        ILoginMessage.setFont(new Font("verdana",Font.ITALIC,18));
        HLoginMessage.setFont(new Font("verdana",Font.ITALIC,18));
        f.add(ILoginMessage);
        f.add(HLoginMessage);
        ILoginButton = new JButton("Login");
        HLoginButton = new JButton("Login");
        ILoginButton.setFont(new Font("verdana",Font.BOLD,18));
        HLoginButton.setFont(new Font("verdana",Font.BOLD,18));
        ...
        ILoginPageTF1.setFont(new Font("verdana",Font.PLAIN,18));
        ILoginPageTF2.setFont(new Font("verdana",Font.PLAIN,18));
        HLoginPageTF1.setFont(new Font("verdana",Font.PLAIN,18));
        HLoginPageTF2.setFont(new Font("verdana",Font.PLAIN,18));
        ILoginPageLabel1.setFont(new Font("verdana",Font.PLAIN,18));
        ILoginPageLabel2.setFont(new Font("verdana",Font.PLAIN,18));
        HLoginPageLabel1.setFont(new Font("verdana",Font.PLAIN,18));
        HLoginPageLabel2.setFont(new Font("verdana",Font.PLAIN,18));
        f.add(ILoginPageTF1);
        f.add(ILoginPageTF2);
        f.add(HLoginPageTF1);
        f.add(HLoginPageTF2);
        ...
        requestDetailsPageLabel1[4].setText("department_name : ");
        ...
        requestDetailsPageRequestNumberLabel.setFont(new Font("verdana",Font.PLAIN,22));
        requestDetailsPageInput = new JTextField();
        requestDetailsPageInput.setFont(new Font("verdana",Font.PLAIN,22));
        requestDetailsPageMessage = new JLabel();
        requestDetailsPageMessage.setFont(new Font("verdana",Font.ITALIC,18));
        requestDetailsPageButton = new JButton("Get Details");
        requestDetailsPageButton.setFont(new Font("verdana",Font.BOLD,18));
        ...
        String department_names[] = {"CSE","ECE","AIE","ME","EAC","EEE"};
        requestPagedepartment_nameComboBox = new JComboBox(department_names);
        requestPageMessage.setFont(new Font("verdana",Font.ITALIC,18));
        requestPageOkButton.addActionListener(this);
        backToHomeButton.addActionListener(this);
        requestPageOkButton.setFont(new Font("verdana",Font.BOLD,18));
        backToHomeButton.setFont(new Font("verdana",Font.BOLD,18));
        requestPageLabels[0].setText("Enter Your Id");
        requestPageLabels[1].setText("Enter Services(Eg:Keyboard repair,Password error)");
        requestPageLabels[2].setText("Select department");
        homeButtons[0].setText("Place Request");
        homeButtons[1].setText("View request details");
        homeButtons[2].setText("Login as ICTS Staff");
        homeButtons[3].setText("Login as HOD");
        homeButtons[4].setText("Login as Admin");
        homeButtons[5].setText("Exit");
        homeButtons[0].setBackground(Color.lightGray);
        homeButtons[1].setBackground(Color.lightGray);
        homeButtons[2].setBackground(Color.lightGray);
        homeButtons[3].setBackground(Color.lightGray);
        homeButtons[4].setBackground(Color.lightGray);
        homeButtons[5].setBackground(Color.lightGray);
        requestPagedepartment_nameComboBox.setFont(new Font("verdana",Font.PLAIN,18));
        f.add(requestPagedepartment_nameComboBox);
        f.add(welcomeLabel);
        f.setSize(1400,900);
        f.setLayout(null);
        f.setVisible(true);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(clicked == "Place Request") {
                ...
                for(int i=0;i<requestNumbers.length;i++) {
                    try {
                        ...
                        if(getIntFromResultSet(ps.executeQuery()) == 0) {
                            ...
                            if(error2 == false) {
                                ps = con.prepareStatement("select department_name from request where request_no = ?");
                                ps.setInt(1,requestNo);
                                rs = ps.executeQuery();
                                rs.next();
                                if(isEqual(Integer.toString(currentHLogin),Integer.toString(rs.getInt("department_name"))) == false) {
                                    error2 = true;
                                }
                            }
                        }
            ...
            if(error1 == true) {
                ...
                department_name temp = new department_name();
                for(int i=0;i<requestNumbers.length;i++) {
            ...
            if(e.getSource() == ILoginButton) {
                ...
                if(isEqual("kirioli",pass) == false || pass.length()!=7) {
            ...
            if(error1 == true) {
                ...
                try {
                    ...
                    requestDetailsPageLabel2[5].setText(temp);
                    temp = "";
                    if(allRequests.elementAt(index).department_name == 1) temp = "CSE";
                    else if(allRequests.elementAt(index).department_name == 2) temp = "ECE";
                    else if(allRequests.elementAt(index).department_name == 3) temp = "AIE";
                    else if(allRequests.elementAt(index).department_name == 4) temp = "ME";
                    else if(allRequests.elementAt(index).department_name == 2) temp = "EAC";
                    else temp = "EEE";
        ...
        else if(clicked == "Login as ICTS Staff") {
            loadICTSLoginPage();
        }
        else if(clicked == "Login as HOD") {
            loadHODLoginPage();
        }
        else if(clicked == "Login as Admin") {
            loadAdminLoginPage();
        }
        else if(clicked == "OK") {
            if(e.getSource() == requestPageOkButton) {
                ...
                int department_name = requestPagedepartment_nameComboBox.getSelectedIndex()+1;
                String Services[] ;
                ...
                if(error1 == true) {
                    ...
                    try {
                        ...
                        ps.setInt(8,Request.department_name);
                    ...
                    if(Request.approvalRequired == 'N') {
                        alldepartment_names[Request.department_name-1].allotJob(Request);
                    }
                    allRequests.add(Request);
                    requestPageMessage.setText("Your request is submitted  (Your request number : " + allRequests.size() + ")");
                }
            }
